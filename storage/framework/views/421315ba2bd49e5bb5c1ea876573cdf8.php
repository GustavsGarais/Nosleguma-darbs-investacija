...
<?php $__env->startSection('title', __('Quick Tour Simulation (read-only template)')); ?>
...
<section class="hero" style="padding:48px 0; display:flex; justify-content:center;">
    <div class="hero-content" style="width:min(1100px, 100% - 32px); margin:0 auto; display:grid; gap:24px;">
        <header style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">
            <div>
                <p style="margin:0; text-transform:uppercase; letter-spacing:0.08em; font-size:12px; color:var(--c-on-surface-2);"><?php echo e(__('Guided demo')); ?></p>
                <h1 style="margin:4px 0 8px;"><?php echo e(__('Quick Tour Simulation (read-only template)')); ?></h1>
                <p style="margin:0; color:var(--c-on-surface-2); max-width:720px;">
                    <?php echo e(__('Explore a pre-made investment scenario. You can tweak the numeric parameters and watch how the portfolio behaves. The name and speed are fixed so you focus on the money dynamics.')); ?>

                </p>
            </div>
            <div style="display:flex; gap:12px; align-items:center;">
                <div style="font-size:13px; color:var(--c-on-surface-2);">
                    <?php echo e(__('Speed fixed at :speed / step', ['speed' => '0.50s'])); ?>

                </div>
                <a class="btn btn-outline" href="<?php echo e(route('simulations.create')); ?>"><?php echo e(__('Create your own')); ?></a>
            </div>
        </header>
        ...
        <div class="auth-card" style="padding:20px; display:grid; gap:20px;">
            <div style="display:flex; flex-wrap:wrap; gap:12px; align-items:center; justify-content:space-between;">
                <div>
                    <p style="margin:0; text-transform:uppercase; letter-spacing:0.06em; font-size:12px; color:var(--c-on-surface-2);"><?php echo e(__('Scenario')); ?></p>
                    <h2 style="margin:4px 0 0;"><?php echo e(__('Starter Balanced Portfolio')); ?></h2>
                    <p style="margin:4px 0 0; color:var(--c-on-surface-2); font-size:13px;"><?php echo e(__('Name is locked in this tour. Adjust values to see impact.')); ?></p>
                </div>
                <div style="display:flex; gap:8px; flex-wrap:wrap;">
                    <button id="qt-run" class="btn btn-primary">▶ <?php echo e(__('Run')); ?></button>
                    <button id="qt-step" class="btn btn-secondary">➜ <?php echo e(__('Step')); ?></button>
                    <button id="qt-pause" class="btn btn-secondary" disabled>⏸ <?php echo e(__('Pause')); ?></button>
                    <button id="qt-reset" class="btn btn-outline">🔄 <?php echo e(__('Reset')); ?></button>
                    <div id="qt-status" style="padding:8px 12px; border:1px solid var(--c-border); border-radius:10px; background:color-mix(in srgb, var(--c-surface) 92%, var(--c-primary) 8%);"><?php echo e(__('Ready')); ?></div>
                </div>
            </div>

            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap:12px;">
                <label style="display:grid; gap:6px;">
                    <span><?php echo e(__('Duration (months)')); ?></span>
                    <input id="qt-months" type="number" min="12" max="600" step="12" value="120" class="footer-email-input" />
                </label>
                <label style="display:grid; gap:6px;">
                    <span><?php echo e(__('Speed (seconds/step)')); ?></span>
            ...
            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap:12px;">
                <div class="auth-card" style="padding:14px;">
                    <p style="margin:0; font-size:12px; color:var(--c-on-surface-2); text-transform:uppercase;"><?php echo e(__('Current Value')); ?></p>
                    <p id="qt-current" style="margin:4px 0 0; font-size:22px; font-weight:700; color:var(--c-primary);">€0.00</p>
                </div>
                <div class="auth-card" style="padding:14px;">
                    <p style="margin:0; font-size:12px; color:var(--c-on-surface-2); text-transform:uppercase;"><?php echo e(__('Contributed')); ?></p>
                    <p id="qt-contrib" style="margin:4px 0 0; font-size:22px; font-weight:700;">€0.00</p>
                </div>
                <div class="auth-card" style="padding:14px;">
                    <p style="margin:0; font-size:12px; color:var(--c-on-surface-2); text-transform:uppercase;"><?php echo e(__('Gain')); ?></p>
                    <p id="qt-gain" style="margin:4px 0 0; font-size:22px; font-weight:700;">€0.00</p>
                </div>
                <div class="auth-card" style="padding:14px;">
                    <p style="margin:0; font-size:12px; color:var(--c-on-surface-2); text-transform:uppercase;"><?php echo e(__('Real (inflation adj.)')); ?></p>
            ...
            <div class="auth-card" style="padding:16px;">
                <div style="display:flex; justify-content:space-between; align-items:center; gap:12px; flex-wrap:wrap;">
                    <h3 style="margin:0;"><?php echo e(__('Simulation preview')); ?></h3>
                    <p style="margin:0; color:var(--c-on-surface-2); font-size:13px;"><?php echo e(__('Speed is fixed to 0.50s per step. Names are locked for this demo.')); ?></p>
                </div>
...
document.addEventListener('DOMContentLoaded', () => {
    const qtChart = <?php echo json_encode($qtChart, 15, 512) ?>;
    ...
    const t = {
        ready: <?php echo json_encode(__('Ready'), 15, 512) ?>,
        running: <?php echo json_encode(__('Running...'), 15, 512) ?>,
        complete: <?php echo json_encode(__('Complete'), 15, 512) ?>,
        paused: <?php echo json_encode(__('Paused'), 15, 512) ?>,
        month: <?php echo json_encode(__('Month :current / :total'), 15, 512) ?>
    };
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\Nosleguma-darbs-investacija\resources\views/pages/quick-tour.blade.php ENDPATH**/ ?>